...
bool elementSum(int num[], int l, int r)
{
    RQuickSort(num,l,r);
    ...
    for(int k=0; k<n; k++)    
    {
        ...
        while(left <= right)
        {
            if(num[left]+num[right] == num[k])
            {
                return true;
            }
            else if(num[left] + num[right] < num[k])
                left++;
            else
                right--;
        }
...
int main()
{
    ...
    if(elementSum(num,0,n-1))
        printf("Yes the sum of two elements exists in the array\n");
    else
        printf("No the sum of two elements does not exist\n");
